@time using LinearAlgebra, Polynomials, Reactive, Interact, Plots
gr()
...
function savegif(y_ary::Matrix{Float64})
    anim = @animate for i = 1:50:length(y_ary[:, 1])
        plot(y_ary[1:i, 1], y_ary[1:i, 2], y_ary[1:i, 3], legend = :none, title = "Lorenz attractor",
        xlabel = "x", ylabel = "y", zlabel = "z", xlims = (-20, 20), ylims = (-25, 25), zlims = (0, 50))
    end
